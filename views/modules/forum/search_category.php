...
<script type="text/javascript" src="<?php echo base_url("js/jquery.selectbox-0.1.3.min.js"); ?>"></script>

<?php $this->load->view('modules/forum/category_navbar'); ?>
 
<div>
	<ul class="my_contacts_pagination" style="margin:15px 0; overflow:visible;">
		<li><?php echo '<a href="'.$pagination['url'].'&page='.$pagination['page'].'">Last Page</a>' ?></li>
		<li class="next <?=$pagination['cur'] >= $pagination['page'] ?'disable':''?>"><?= $pagination['cur'] >= $pagination['page'] ? '<a href="#"></a>' : '<a href="'.$pagination['url'].'&page='.($pagination['cur']+1).'"></a>' ?></li>
		<li class="prev <?=$pagination['cur'] <=1?'disable':''?>"><?= $pagination['cur'] <= 1 ? '<a href="#"></a>' : '<a href="'.$pagination['url'].'&page='.($pagination['cur']-1).'"></a>' ?></li>
...
<?php if(count($topics) && is_array($topics)): ?>
	...
	<div class="forum_block_item<?=($i%2==0) ? ' white' : '' ?>">
		...
		<div class="topics">
			<h3><?=intval($topic->views)?> / <?=intval($topic->replies)?></h3>
		</div>
	</div>
	<?php endforeach; ?>
<?php else: ?>
...
$(document).ready(function(){
	$('select.pagination').selectbox({
		onChange: function(val,inst){
			window.location = '<?=$pagination['url']?>&page='+val;
		}
	})
});
</script>
